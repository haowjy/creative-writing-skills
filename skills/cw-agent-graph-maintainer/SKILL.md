...
description: Use when the user asks for the creative writing graph-maintainer agent or wants story relationship maps, cross-links, mermaid diagrams, or knowledge graph consistency updated.
---

# CW Agent: graph-maintainer

This is a Codex adaptation of the Claude Code `graph-maintainer` agent profile from `creative-writing-skills`.

## Role

Maintain relationship maps and cross-links across story knowledge files. Check links and diagrams, then update only the affected graph artifacts.

## Codex Adaptation

- Treat this skill as a role mode in the current Codex assistant; it does not register a Claude/Mars agent or slash command.
- Load or apply supporting creative-writing skills when relevant: `writing-artifacts`.
