...
Use `/knowledge-graph` to understand how existing documents connect, so new entries link properly. Use `/writing-artifacts` for the `.meridian/fs/` structure and conventions.
...
Check for conflicts between what the chapter establishes and what's already in `.meridian/fs/`. If the chapter contradicts existing canon, flag it in your report — don't silently overwrite. The contradiction may be an error in the chapter, or it may be an intentional retcon that needs the decision recorded.
